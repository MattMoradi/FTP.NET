...
namespace Client
{
    public static class Connection
    {
        ...
        public static int Connect(ref ClientWrapper client, ref Logger logger, Commands.Connect commands, ref Program.FilePath path)
        {
            if (commands.IP == null)
            {
                Console.WriteLine("ERROR: must provide a host name to connect!");
            ...
            string password = String.Empty;
            ConsoleKey key;
            client.internalClient.Host = commands.IP;
            bool firstAuth = false;
            ...
            if (File.Exists(commands.IP + ".txt"))
            {
                try
                {
                    ...
                    using (StreamReader Reader = new StreamReader(credFile))
                    {
                        client.internalClient.Credentials.UserName = Reader.ReadLine();
                        password = Reader.ReadLine();
                        Reader.Close();
                    };
                    ...
                    ICryptoTransform cryptoTransform = cipher.CreateDecryptor();
                    byte[] pass = Convert.FromBase64String(password);
                    client.internalClient.Credentials.Password = Encoding.UTF8.GetString(cryptoTransform.TransformFinalBlock(pass, 0, pass.Length));
                    client.internalClient.AutoConnect();
                }
                catch (Exception x)
                {
                    Console.WriteLine(x.ToString());
            ...
            {
                Console.Write("Enter the username: ");
                client.internalClient.Credentials.UserName = Console.ReadLine();
                Console.Write("Enter the password: ");
                firstAuth = true;
                ...
                client.internalClient.Credentials.Password = password;
                Console.WriteLine();
            }
            ...
            if (client.IsAuthenticated())
            {
                ...
                logger = new Logger(client.internalClient.Credentials.UserName);
                Console.WriteLine("Successfully connected to server!");
                return 0;
            }
            else
        ...
        public static int Save(ref ClientWrapper client)
        {
            string? credsFile = client.internalClient.Host + ".txt";
            ...
            try
            {
                ...
                cipher.IV = Convert.FromBase64String(iv);
                ICryptoTransform cryptoTransform = cipher.CreateEncryptor();
                byte[] pass = Encoding.UTF8.GetBytes(client.internalClient.Credentials.Password);

                StreamWriter credentials = new StreamWriter(credsFile);
                credentials.WriteLine(client.internalClient.Credentials.UserName);
                credentials.WriteLine(Convert.ToBase64String(cryptoTransform.TransformFinalBlock(pass, 0, pass.Length)));
                credentials.Close();
                return 0;
            }
        ...
        public static void Timeout(ref ClientWrapper client)
        {

        }

        public static int Disconnect(ref ClientWrapper client, ref Logger? logger)
        {
            client.Disconnect();
            logger = null;
            Console.WriteLine("--- All Connections Terminated ---\n");
