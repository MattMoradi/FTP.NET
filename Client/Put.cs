...
namespace Client
{
	public static class Put
	{
		public static int File(ref ClientWrapper client, Commands.Put file, in Program.FilePath path)
		{
			try
			{
				if (!client.IsAuthenticated())
		...
		public static int SingleFile(ref ClientWrapper client, string[] files, in Program.FilePath path, ProgressBarOptions options)
		{
			string localPath = files[0];
			string fullRemotePath = path.Remote + Path.GetFileName(localPath);
		...
		public static int MultipleFiles(ref ClientWrapper client, string[] files, in Program.FilePath path, ProgressBarOptions options)
		{
			Console.WriteLine("Uploading (" + files.Count() + ") files to: " + path.Remote);
		...
		public static int Create(ref ClientWrapper client, Commands.CreateDirectory directory)
		{
			if (client.IsAuthenticated())
			{
				if (client.DirectoryExists(directory.Name))
		...
		public static int Copy(ref ClientWrapper client, Commands.Copy file)
		{
			return 0;
		}
	}
